...
- The **Ready to leave** check above **Filter bank** compares the selected setup
  ...
  **Charges & looting bag** shows automatic **Weapon Charges** estimates and your
  ...
  [departure check notes](reconstruction/ready-to-leave.md).
...
- **Melee boosts**, **Ranged boosts** and **Magic boosts** in AIO settings let
  ...
  **Boost slots** and list **Preferred boosts** in priority order, separated by
  ...
  automatic packing. See the [boost reservation notes](reconstruction/boost-reservations.md).
- **Food > Override food** applies one custom food preference to all combat
  ...
  [food override notes](reconstruction/food-overrides.md) for supported forms.
...
[teleport source notes](reconstruction/return-teleport-sources.md).
...
teleports, unlock knowledge and search cutoff still apply. AIO shows "Route sent";
check Shortest Path for the route result, including any unreachable destination.
The packing list includes bundled travel, return and escape
...
[integration notes](reconstruction/shortest-path-integration.md).
...
remains authoritative. See the [rule evidence and limits](reconstruction/wilderness-risk-sources.md).
...
[coverage report](reconstruction/data-coverage.md) for the exact boundaries
